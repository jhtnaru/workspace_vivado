...
module watch (
    input clk, reset_p,
    input set_watch, inc_hour, inc_min, clear,
    output reg [7:0] hour, min, sec, csec
    );

    reg [26:0] cnt_sysclk = 1;                          // Clock Division

    wire inc_hour_pedge, inc_min_pedge, clear_pedge;
    edge_detector_pos inc_hour_ed (.clk(clk), .reset_p(reset_p), .cp(inc_hour), .p_edge(inc_hour_pedge));
    edge_detector_pos inc_min_ed  (.clk(clk), .reset_p(reset_p), .cp(inc_min), .p_edge(inc_min_pedge));
    ...
    always @(posedge clk, posedge reset_p) begin
        if (reset_p) begin                          // Time Reset
            ...
            if (set_watch) begin                    // Set Mode
                if (inc_hour_pedge) begin
                    if (hour >= 23) hour <= 0;
                    else hour <= hour + 1;
                end
                if (inc_min_pedge) begin
                    if (min >= 59) min <= 0;
                    else min <= min + 1;              // Minute Increase
                end
            end
            else begin                              // Start Mode
                if (cnt_sysclk >= 27'd1_000_000) begin
                    cnt_sysclk <= 1;
                    if (csec >= 99) begin
                        csec <= 0;
                        if (sec >= 59) begin
