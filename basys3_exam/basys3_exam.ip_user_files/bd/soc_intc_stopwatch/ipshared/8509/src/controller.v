...
// Stop Watch Module, Register 입력 방식으로 변경
module stop_watch (
    input clk, reset_p,
    input start_stop, lap, clear,
    output reg [7:0] hour, min, sec, csec,
    output reg [7:0] lap_hour, lap_min, lap_sec, lap_csec
    );
    
    reg [26:0] cnt_sysclk = 1;                          // Clock Division Count
    ...
    always @(posedge clk, posedge reset_p) begin
        ...
        else if (lap_pedge && (csec != 0 || sec != 0 || min != 0 || hour != 0)) begin
            lap_hour <= hour;
            lap_min <= min;
            ...
            if (start_stop) begin
                if (cnt_sysclk >= 1_000_000) begin  // 1ns * 1,000,000 = 0.01s
                    ...
                    if (csec >= 99) begin           // 0 ~ 99 Sub Second Count
                        csec <= 0;
                        if (sec >= 59) begin        // 0 ~ 99 Second Count
                            sec <= 0;
                            if (min >= 59) begin
                                min <= 0;
                                if (hour >= 99) begin
